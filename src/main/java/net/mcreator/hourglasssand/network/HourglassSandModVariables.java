...
public class HourglassSandModVariables {
	...
	public static class WorldVariables extends SavedData {
		public static final SavedDataType<WorldVariables> TYPE = new SavedDataType<>("hourglass_sand_worldvars", ctx -> new WorldVariables(), ctx -> CompoundTag.CODEC.xmap(tag -> {
	...
	public static class MapVariables extends SavedData {
		public static final SavedDataType<MapVariables> TYPE = new SavedDataType<>("hourglass_sand_mapvars", ctx -> new MapVariables(), ctx -> CompoundTag.CODEC.xmap(tag -> {
			MapVariables instance = new MapVariables();
			instance.read(tag, ctx.levelOrThrow().registryAccess());
			return instance;
		}, instance -> instance.save(new CompoundTag(), ctx.levelOrThrow().registryAccess())), null);
		boolean _syncDirty = false;
		public double Status = 1.0;
		public boolean StopStatus = false;

		public void read(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			Status = nbt.getDoubleOr("Status", 0);
			StopStatus = nbt.getBooleanOr("StopStatus", false);
		}

		public CompoundTag save(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			nbt.putDouble("Status", Status);
			nbt.putBoolean("StopStatus", StopStatus);
			return nbt;
		}
	...
	public record SavedDataSyncMessage(int dataType, SavedData data) implements CustomPacketPayload {
		public static final Type<SavedDataSyncMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(HourglassSandMod.MODID, "saved_data_sync"));
